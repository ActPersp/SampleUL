# The scripts below run discounted cash flow model for a Plan.UL1 policy.
...
cov <- Cov(
   ...
   issAge = 40L,                      # issue age
   riskClass = "MN",                  # risk class: male nonsmoker
   faceAmt = 100000,                  # face amount
   premMode = 1L,                     # premium mode: annual
   modPrem = 1200                     # modal premium
)
...
saveWorkbook(wb, "export/UL1.xlsx", overwrite = TRUE)
